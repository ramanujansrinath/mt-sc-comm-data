function dim = crossvalidate_fa2(X, varargin)
%
...
%           L=estParams.L;
%           LLt=L*L';
%           dim(i).rsq(cvf+1)=1-sum((LLt(U==1)-COV(U==1)).^2)./((length(find(U==1))-1)*var(COV(U==1)));
      else
...
%               dim(i).rsq(cvf+1)=1-sum((LLt(U==1)-COV(U==1)).^2)./((length(find(U==1))-1)*var(COV(U==1)));
%               COV0=cov(Xtrain');
%               dim(i).rsq_data(cvf)=1-sum((COV0(U==1)-COV(U==1)).^2)./((length(find(U==1))-1)*var(COV(U==1)));
          end
